        ...
        @if($permohonan->parent_permohonan_id)
            <div class="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4">
                <p class="text-xs text-amber-700">
                    <strong>Pengajuan Ulang</strong> — Permohonan ini diajukan ulang dari
                    <a href="{{ route('pemohon.permohonan.show', $permohonan->parent_permohonan_id) }}" class="underline font-mono">{{ $permohonan->pengajuanAsal->no_registrasi ?? $permohonan->parent_permohonan_id }}</a>
                </p>
            </div>
        @endif
...
{{-- Dokumen --}}
<x-ui.card class="mb-4">
    <x-ui.card-header title="Dokumen" description="Dokumen yang telah diupload" />
...
@if(in_array($permohonan->status_saat_ini, [\App\Models\Permohonan::STATUS_REVISI_1, \App\Models\Permohonan::STATUS_REVISI_2, \App\Models\Permohonan::STATUS_REVISI_3]))
    @if($permohonan->evaluasiTerakhir)
    <x-ui.card class="mb-4 border-l-4 border-l-amber-500">
        <x-ui.card-header title="Catatan Revisi" description="Staff menemukan hal yang perlu diperbaiki" />
        <x-ui.card-content>
            <p class="text-sm text-slate-700 whitespace-pre-wrap">{{ $permohonan->evaluasiTerakhir->catatan ?: 'Tidak ada catatan spesifik.' }}</p>
        </x-ui.card-content>
    </x-ui.card>
    @endif

    <x-ui.button variant="default" href="{{ route('pemohon.revisi.show', $permohonan) }}" class="mb-4">
        📤 Upload Revisi
    </x-ui.button>
@endif
